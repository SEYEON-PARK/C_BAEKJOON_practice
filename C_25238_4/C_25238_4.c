/*
메이플스토리 몬스터는 방어율 수치가 있습니다. 
이 방어율 수치의 일정 %를 무시하는 것을 방무라고 합니다. 유저는 아이템을 사거나, 특정한 스킬 레벨을 올려서 방무 수치를 올릴 수 있습니다. 
...
int main(void)
{
    ...
    if(a-a*(b/(float)100)>=100) // 만약, a-a*(b/(float)100)의 값이 100보다 크거나 같다면
        printf("0"); // "0" 출력하기
    else // a-a*(b/(float)100)의 값이 100보다 작다면
        printf("1"); // "1" 출력하기
